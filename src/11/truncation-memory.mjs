...
async function summarizationMemoryDemo() {
  ...
  if (allMessages.length >= maxMessages) {
    ...
    const answer = await askAi("用户在做什么菜？", await history.getMessages());
    console.log(`\nAI 回答: ${answer}`);
  } else {
    console.log("\n消息数量未超过阈值，无需总结");
  }
...
async function summarizeHistory(messages) {
  ...
  const summaryPrompt = `请总结以下对话的核心内容，保留菜名和做法：
...
async function askAi(question, msg) {
  const messages = [
    new SystemMessage(`
            基于用户提供的上线问回答问题，如果上下文没有提及，就如实回答没有提及。
            上下文：${msg}
        `),
    ...msg,
